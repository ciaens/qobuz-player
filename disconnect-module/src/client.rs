...
use disconnect_server_module::DisconnectServerEvent;
...
impl DisconnectClient {
    ...
    pub async fn get_state(&self) -> AppResult<disconnect_server_module::DisconnectState> {
        let res = self
            .client
            .get(self.url("/state"))
            .send()
            .await?
            .json::<disconnect_server_module::DisconnectState>()
            .await?;
